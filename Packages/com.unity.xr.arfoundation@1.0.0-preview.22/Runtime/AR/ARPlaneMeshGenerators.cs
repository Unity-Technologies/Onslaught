...
namespace UnityEngine.XR.ARFoundation
{
    ...
    public static class ARPlaneMeshGenerators
    {
        ...
        /// <param name="pose">The <c>Pose</c> of the plane, in session space.</param>
        ...
        public static bool GenerateMesh(Mesh mesh, Pose pose, Vector3 center, Vector3 normal, List<Vector3> convexPolygon, float areaTolerance = 1e-6f)
        {
            if (mesh == null)
            ...
            //
            s_Vertices.Clear();
            s_Vertices.Capacity = convexPolygon.Count + 1;
            s_Vertices.AddRange(convexPolygon);
            ...
            // UVs
            //
            GenerateUvs(s_Uvs, pose, normal, s_Vertices);
            mesh.SetUVs(0, s_Uvs);
        ...
        public static void GenerateUvs(List<Vector2> Uvs, Pose pose, Vector3 normal, List<Vector3> vertices)
        {
            ...
            var projection = Vector3.Project(rotationAxis, planeRotation * normal);
        ...
        /// <summary>
        /// Generates a <c>List<int></c> of indices from the given parameters. The <paramref name="convexPolygon"/> is assumed to be convex.
        /// </summary>
        /// <remarks>
        /// <paramref name="convexPolygon"/> is not checked for its convexness. Concave polygons will produce incorrect results.
        /// </remarks>
